...
class DNN:
    ...
    def learn(self, pose, pose_old, command_old):
        ...
        if self.buffer1.current_index >= self.buffer1.buffer_size:
            x_batch, y_batch = self.cache1.load_batch()  # tensor

            # Run the training
            self.agent1.train(x_batch, y_batch)

        if self.buffer2.current_index >= self.buffer2.buffer_size:
