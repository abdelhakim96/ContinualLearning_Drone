...
from agents.utils import AverageMeter, get_accuracy, cutmix_data, get_logger, seed_everything, save_model, load_model
...
def experiment(opt, class_mask, train_loader, test_loader, model, logger, num_passes):
    ...
    for epoch in range(num_passes):
        ...
        # Train and test loop
        logger.info(
            "==> Starting pass number: " + str(epoch) + ", Learning rate: " + str(optimizer.param_groups[0]['lr']))
        model, optimizer = train(opt=opt, loader=train_loader, model=model, criterion=criterion, optimizer=optimizer,
                                 epoch=epoch, logger=logger)
        prec1 = test(loader=test_loader, model=model, criterion=criterion, class_mask=class_mask, logger=logger,
                     epoch=epoch)
...
def train(opt, loader, model, criterion, optimizer, epoch, logger):
    ...
    for inputs, labels in loader:
        ...
        outputs = model(inputs)
        loss = lam * criterion(outputs, labels_a) + (1 - lam) * criterion(outputs,
                                                                          labels_b) if do_cutmix else criterion(outputs,
                                                                                                                labels)
        optimizer.zero_grad()
        loss.backward()
...
if __name__ == '__main__':
    ...
    if opt.num_pretrain_classes > 0:
        ...
        if not os.path.isfile(opt.log_dir + opt.old_exp_name + '/pretrained_model.pth.tar'):
            ...
            _, model = experiment(opt=opt, class_mask=torch.ones(opt.num_classes, opt.num_classes).cuda(),
                                  train_loader=dobj.pretrain_loader, \
                                  test_loader=dobj.pretest_loader, model=model, logger=console_logger,
                                  num_passes=opt.num_pretrain_passes)
            save_model(opt,
                       model)  # Saves the pretrained model. Subsequent CL experiments directly load the pretrained model.
        else:
    ...
    best_acc1, model = experiment(opt=opt, class_mask=dobj.class_mask, train_loader=dobj.cltrain_loader,
                                  test_loader=dobj.cltest_loader, \
