...
class SphereDataset(Dataset):
    def __init__(self, dataset_name, type_scaling):
        ...
        data = pd.read_csv('data/log_' + dataset_name + '.csv',
                           usecols=['x', 'y', 'yaw', 'v_x', 'v_y', 'w_z', 'tau_x', 'tau_y', 'tau_z'])
        ...
        data[['diff_x(k+2)', 'diff_y(k+2)']] = data[['x', 'y']].values[3:] - data[['x', 'y']][:-3]
        ...
        header = ['I:diff_x(k)', 'I:diff_y(k)', 'I:diff_x(k+1)', 'I:diff_y(k+1)', 'I:diff_x(k+2)', 'I:diff_y(k+2)',
                  'I:sin', 'I:cos',
                  'I:v_x', 'I:v_y', 'I:w_z',
                  'O:tau_x', 'O:tau_y', 'O:tau_z']
        dataframe = pd.DataFrame(data[['diff_x(k)', 'diff_y(k)', 'diff_x(k+1)', 'diff_y(k+1)', 'diff_x(k+2)', 'diff_y(k+2)',
                                       'sin', 'cos', 'v_x', 'v_y', 'w_z',
                                       'tau_x', 'tau_y', 'tau_z']].values,
                                 columns=header)
        ...
        self.input = data[['diff_x(k)', 'diff_y(k)', 'diff_x(k+1)', 'diff_y(k+1)', 'diff_x(k+2)', 'diff_y(k+2)',
                           'sin', 'cos',
                           'v_x', 'v_y', 'w_z']].values
        self.output = data[['tau_x', 'tau_y', 'tau_z']].values
        ...
        with open('models/sphere.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    numHiddenUnits = [64, 64]
    maxEpochs = 100

    datasetName = 'sphere_random'
    networkName = 'dnn_' + 'x'.join([str(s) for s in numHiddenUnits])
    ...
    dataset = SphereDataset(datasetName, typeScaling)
